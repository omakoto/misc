...
def show_mixer(name, kwargs):
    ...
    volumes = mixer.getvolume(alsaaudio.PCM_PLAYBACK)
    for i in range(len(volumes)):
        print("Channel %i playback volume: %i%%" % (i,volumes[i]))

    volumes = mixer.getvolume(alsaaudio.PCM_CAPTURE)
    for i in range(len(volumes)):
        print("Channel %i capture volume: %i%%" % (i,volumes[i]))
