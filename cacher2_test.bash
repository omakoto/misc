#!/bin/bash
#
# cacher2_test.bash - Integration test suite for cacher2.
# Run this test script to verify cacher2 functionality.
#

. testutil.bash

# Temporary directory for tests
TEST_DIR=$(mktemp -d -t cacher2_test_XXXXXX)
...
# Test 1: Help message
assert "./cacher2 -h 2>&1 | grep -q 'Usage of cacher'"
assert "./cacher2 --help 2>&1 | grep -q 'Usage of cacher'"

# Test 2: First run with default text
clear_files
# Run cacher2. It should:
# 1. Immediately return "default_val"
# 2. Start the command (which sleeps 1 then prints "new_val") in background
echo -n "default_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n new_val" -f "$CACHE_FILE" -d "default_val"
...
clear_files
# Run cacher2 without -d. It should return "?" immediately
echo -n "?" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n new_val" -f "$CACHE_FILE"
...
echo -n "cached_val" | assert_out -d -- ./cacher2 -c "echo -n newer_val" -f "$CACHE_FILE" -a 10
sleep 0.5
...
clear_files
echo -n "cached_val" > "$CACHE_FILE"
echo -n "cached_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n updated_val" -f "$CACHE_FILE" -a 0
...
./cacher2 -c "sleep 2 && echo -n first_bg" -f "$CACHE_FILE" -a 0 &
# Wait a bit to let it acquire the lock and start running
sleep 0.5

# Now run a second cacher2 with command to update to "second_bg"
# It should print "init_val" (since lock is held, it won't run "second_bg")
echo -n "init_val" | assert_out -d -- ./cacher2 -c "sleep 2 && echo -n second_bg" -f "$CACHE_FILE" -a 0
...
# Run cacher2 with -v, capturing stderr
./cacher2 -c "echo -n new_val" -f "$CACHE_FILE" -v 2> "$VERBOSE_LOG"
...
./cacher2 -c "sleep 10 && echo -n long_bg" -f "$CACHE_FILE" -a 0 &
sleep 0.5

# Run a second cacher2 with timeout 0 (instant timeout)
...
echo -n "init_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n new_bg" -f "$CACHE_FILE" -a 0 -t 0
...
./cacher2 -c "echo -n command_error >&2" -f "$CACHE_FILE" --show-stderr 2> "$STDERR_OUT"
sleep 0.5
...
echo -n "fresh_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n forced_fresh" -f "$CACHE_FILE" -a 100 -F
...
echo -n "init_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n forced_bg" -f "$CACHE_FILE" -a 0 --force
...
clear_files
# Run cacher2 in foreground mode. It should block for 1 second and then output "foreground_val"
# directly to stdout.
echo -n "foreground_val" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n foreground_val" -f "$CACHE_FILE" -g
...
echo -n "foreground_killed_bg" | assert_out -d -- ./cacher2 -c "sleep 1 && echo -n foreground_killed_bg" -f "$CACHE_FILE" -g
