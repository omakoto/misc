...
class Node
  ...
  OP_MAYBE = :maybe
  OP_NEXT_MUST = :must
  OP_FOR_NEXT_WORD = :for_next_word
  OP_NEXT_WORD = :next_word
...
# But we don't want to execute lazy enums yet. Hmm.
  ...
  def set_next(node)
    node or die "node can't be nil."
    @next and die "next already set."
    @next = node
  end

  def set_first(node)
    node or die "node can't be nil."
    @first and die "next already set."
    @first = node
  end

  def dump(indent="")
    puts indent + self.to_s
    @first.dump(indent + "  ") if @first
    @next.dump(indent) if @next
  end
  ...
  def to_s()
    ...
    if @args == nil
      ...
      if @args.respond_to? :each
        sep = ""
        @args.each {|x| ret << sep; ret << (x ? x.to_s : "nil"); sep = ", "}
      else
        ret << @args.to_s
      end
    end
    ret << " (has block)" if @block
    return ret
  end
end
...
class Completer
  ...
  def _add_node(node)
    if @last_node
      ...
      @parent_nodes[-1].set_first(node)
      @last_node = node
    end
  end

  def _run_child(&block)
    return unless block
    @parent_nodes.push(@last_node)
    @last_node = nil

    block.call()

    @last_node = @parent_nodes.pop
  end

  def for_next_word(match=nil, &block)
    block or die "block must be provided."
    _add_node Node.new(Node::OP_FOR_NEXT_WORD, match, &block)

    _run_child(&block)
  end

  def next_word(&block)
    _add_node Node.new(Node::OP_NEXT_WORD, &block)

    _run_child(&block)
  end

  def maybe(*args, &block)
    _add_node Node.new(Node::OP_MAYBE, args, &block)

    _run_child(&block)
  end

  def next_must(*args, &block)
    _add_node Node.new(Node::OP_NEXT_MUST, args, &block)

    _run_child(&block)
  end

  def break_loop()
    block_given? and die "doesn't take block."
    _add_node Node.new(Node::OP_BREAK_LOOP)
  end

  def finish()
    block_given? and die "doesn't take block."
    _add_node Node.new(Node::OP_FINISH)
  end

  def label(label_name, &block)
    block or die "block must be provided."
    _add_node Node.new(Node::OP_LABEL, label_name, &block)
  end

  def jump(label_name)
    block_given? and die "doesn't take block."
    _add_node Node.new(Node::OP_JUMP, label_name)
  end
