...
def prepare_remapper(remapper: BaseRemapper) -> None:
    global debug, quiet
    ...
    def detect_devices():
        ...
        for device in devices:
            if remapper.grab_devices and not _try_grab(device):
                print(f"  Unable to grab, skipping device {device}", file=sys.stderr)
                continue
            reading_devices.append(device)
    ...
    glib.io_add_watch(udev_monitor.fileno(), glib.IO_IN, detect_devices)
...
class SimpleRemapper(BaseRemapper):
    ...
    def start(self, args):
        ...
        prepare_remapper(self)

        self.tray_icon.run() # Start GFK main loop
