...
# Test 1: Help message
assert "$CACHER_BIN -h 2>&1 | grep -q 'Cache command stdout'"
assert "$CACHER_BIN --help 2>&1 | grep -q 'Cache command stdout'"
...
echo -n "cached_val" | assert_out -d -- $CACHER_BIN -c "echo -n newer_val" -f "$CACHE_FILE" -a 10
sleep 0.5
...
clear_files
echo -n "cached_val" > "$CACHE_FILE"
echo -n "cached_val" | assert_out -d -- $CACHER_BIN -c "sleep 1 && echo -n updated_val" -f "$CACHE_FILE" -a 0
...
$CACHER_BIN -c "sleep 2 && echo -n first_bg" -f "$CACHE_FILE" -a 0 &
...
$CACHER_BIN -c "echo -n command_error >&2" -f "$CACHE_FILE" --show-stderr 2> "$STDERR_OUT"
sleep 0.5
...
echo -n "fresh_val" | assert_out -d -- $CACHER_BIN -c "sleep 1 && echo -n forced_fresh" -f "$CACHE_FILE" -a 100 -F
...
echo -n "init_val" | assert_out -d -- $CACHER_BIN -c "sleep 1 && echo -n forced_bg" -f "$CACHE_FILE" -a 0 --force
...
echo -n "foreground_killed_bg" | assert_out -d -- $CACHER_BIN -c "sleep 1 && echo -n foreground_killed_bg" -f "$CACHE_FILE" -g
